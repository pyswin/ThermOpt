"""SAU-FNO standalone inference demo.

SAU-FNO = U-FNO + an axial self-attention block after the last U-Fourier layer.
Loads the bundled SAU-FNO model + normalizers, runs the sample in
data/input_sample.npy, writes the predicted temperature field to
data/predicted_output.npy, and prints a quick comparison against
data/output_sample.npy (the ground truth) if present.

Self-contained: depends only on the files in this folder (model.pt, sau_fno.py,
ufno.py, models/normalize.py) + torch/numpy. No access to the training project
or dataset needed.
...
#   model class      -> sau_fno.SAUNet3d          (./sau_fno.py, subclasses ufno.SimpleBlock3d)
#   normalizer class -> models.normalize.normalize (./models/normalize.py)
HERE = os.path.dirname(os.path.abspath(__file__))
sys.path.insert(0, HERE)
from sau_fno import SAUNet3d            # noqa: F401  (registers module path for torch.load)
from ufno import SimpleBlock3d          # noqa: F401  (SAU-FNO subclasses it)
...
def main():
    ...
    print(f"[loaded] SAU-FNO, {n_params/1e6:.2f}M params")
