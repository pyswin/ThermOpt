...
class SimpleBlock3d(nn.Module):
    def __init__(self, modes1, modes2, modes3, width, in_channels=12):
        ...
        input shape: (batchsize, x, y, z, c=in_channels)
        output shape: (batchsize, x, y, z, c=1)
        """
        self.modes1 = modes1
        self.modes2 = modes2
        self.modes3 = modes3
...
class Net3d(nn.Module):
    ...
    def forward(self, x):
        # x: (batchsize, X, Y, Z, C). Pad each spatial dim up to the next multiple
        # of 8 (the U-Net path does 3 stride-2 down-samples), run, then crop back to
        # the original spatial size. Replaces the original fixed +8 pad / :-8 crop,
        # which assumed 200x96x24 and breaks for Z=1 (pads to 9, not divisible by 8).
        # NOTE: F.pad 'replicate' only reaches the last 3 dims, so X is padded
        # separately with 0 (as in the original).
        batchsize = x.shape[0]
        size_x, size_y, size_z = x.shape[1], x.shape[2], x.shape[3]
        px = (8 - size_x % 8) % 8
        py = (8 - size_y % 8) % 8
        pz = (8 - size_z % 8) % 8
        x = F.pad(x, (0, 0, 0, pz, 0, py), "replicate")             # C, Z, Y
        if px:
            x = F.pad(x, (0, 0, 0, 0, 0, 0, 0, px), 'constant', 0)  # C, Z, Y, X
        x = self.conv1(x)
        x = x.view(batchsize, size_x + px, size_y + py, size_z + pz, 1)
        x = x[:, :size_x, :size_y, :size_z, :]                      # crop back to original
        return x.squeeze(-1)                                        # (batchsize, X, Y, Z)
