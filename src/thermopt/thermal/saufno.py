...
def _default_demo_root() -> Path:
    return Path(__file__).with_name("saufno_demo")


@dataclass
class SAUFNOThermalBackend:
    """SAU-FNO thermal backend = U-FNO + an axial self-attention block.

    Structurally identical to UFNOThermalBackend: the saved model bundle is
    ``[x_normalizer, model, y_normalizer]`` and the forward path is the same.
    The only differences are the demo directory (``saufno_demo/``), the model
    module (``sau_fno.SAUNet3d``, which subclasses ``ufno.SimpleBlock3d``), and
    the ``saufno_*`` config keys.
    """
    ...
    def __post_init__(self) -> None:
        ...
        demo_root_cfg = self.config.get("saufno_demo_root")
        model_path_cfg = self.config.get("saufno_model_path")
    ...
    def _resolve_device(self, torch_module) -> str:
        requested = str(self.config.get("saufno_device", self.config.get("device", "auto"))).lower()
        if requested == "auto":
            return "cuda" if torch_module.cuda.is_available() else "cpu"
        if requested == "cuda" and not torch_module.cuda.is_available():
            raise RuntimeError("SAU-FNO backend requested cuda, but torch.cuda.is_available() is false")
        if requested not in {"cpu", "cuda"}:
            raise ValueError("saufno_device must be one of: auto, cpu, cuda")
        return requested

    def _load_real_predictor(self) -> Callable[[np.ndarray], np.ndarray]:
        saufno_py = self._demo_root / "sau_fno.py"
        ufno_py = self._demo_root / "ufno.py"
        normalize_py = self._demo_root / "models" / "normalize.py"
        if not saufno_py.is_file():
            raise FileNotFoundError(f"missing SAU-FNO demo model file: {saufno_py}")
        if not ufno_py.is_file():
            raise FileNotFoundError(f"SAU-FNO depends on U-FNO model file: {ufno_py}")
        if not normalize_py.is_file():
            raise FileNotFoundError(f"missing SAU-FNO normalizer file: {normalize_py}")
        if not self.model_path or not self.model_path.is_file():
            raise FileNotFoundError(f"missing SAU-FNO model bundle: {self.model_path}")

        try:
            import torch
        except ImportError as exc:
            raise RuntimeError("SAU-FNO backend requires the optional torch dependency.") from exc

        if str(self._demo_root) not in sys.path:
            sys.path.insert(0, str(self._demo_root))

        try:
            import sau_fno as _sau_module  # noqa: F401  (SAUNet3d; pulls in ufno.SimpleBlock3d)
            ...
            raise RuntimeError(f"failed to load SAU-FNO demo modules from {self._demo_root}: {exc}") from exc
        ...
        if not isinstance(model_bundle, (list, tuple)) or len(model_bundle) != 3:
            raise ValueError(f"unexpected SAU-FNO model bundle structure in {self.model_path}")
        ...
        def predict(input_phys: np.ndarray) -> np.ndarray:
            ...
            if input_phys.shape != (*SURROGATE_NATIVE_GRID_SIZE, 1, 3):
                raise ValueError(
                    f"SAU-FNO input must have shape (64, 64, 1, 3), got {input_phys.shape}"
                )
    ...
    def simulate(self, case: FloorplanCase, layout: Layout) -> np.ndarray:
        if case != self.case:
            raise ValueError("SAUFNOThermalBackend is bound to a different case")
        ...
        if temperature.ndim != 2:
            raise ValueError(f"SAU-FNO predictor must return a 2D temperature map, got {temperature.shape}")
