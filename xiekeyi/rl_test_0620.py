...
class _EfficientPlaceEnv:
    def __init__(
        ...
        self.case = case
        self.order = order
        self.train_count = min(max(1, train_count), len(order))
        self.grid = grid
    ...
    def is_done(self) -> bool:
        return self.t >= self.train_count or self.t >= len(self.order)
    ...
    def complete_greedy(self) -> int:
        accepted = 0
        while self.t < len(self.order):
            state = self._observation()
            valid_actions = np.flatnonzero(state[2].reshape(-1) < 0.5)
            if valid_actions.size == 0:
                self._place_with_fallback(self.order[self.t])
                self.t += 1
                continue
            wire_values = state[1].reshape(-1)[valid_actions]
            action = int(valid_actions[int(np.argmin(wire_values))])
            cell_x = action // self.grid
            cell_y = action % self.grid
            self._place(self.order[self.t], cell_x, cell_y)
            self.t += 1
            accepted += 1
        self.state = self._observation()
        return accepted
...
def optimize(
    ...
    train_count = min(len(order), max(1, int(config.get("num_chiplets_to_place", len(order)))))
    rollout_steps = max(1, int(config.get("rollout_steps", train_count)))
    ...
    for episode in range(episodes):
        ...
        greedy_completed = env.complete_greedy()
        attempted_actions += greedy_completed
        accepted_actions += greedy_completed
